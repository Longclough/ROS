...
def post_requirements(test_data_json):
    headers = {'Content-Type': 'application/json' }
    client = TestClient(fast_api)
    response = client.post("/routes/requirements", headers=headers, data=json.dumps(test_data_json, indent=4))
    return response

@responses.activate
def test_requirements_required_services():
    responses.reset
    ...
    post_requirements(test_data_json)
...
def test_requirements_results():
    ...
    test_data_json = load_test_data()
    response = post_requirements(test_data_json)
    ...
    for call in responses.calls:
        if call.request.url.startswith(route_url):
            route = json.loads(call.request.body)
            check_route(route, requirements)

def check_route(route, requirements):
    requirement_ids = []
    for requirement in requirements:
        requirement_ids.append(requirement['id'])

    routepoints = route['routepoint']
...
@responses.activate
def test_requirements_calculate_leg_failed():
    responses.reset
...
@responses.activate
def test_requirements_spatial_validation_failed():
...
@responses.activate
def test_requirements_performance_validation_failed():
    test_data_json = load_test_data()
    requirements = test_data_json['requirement']
