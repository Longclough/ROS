...
import __main__
import testsupport.mqtt5_props
...
def gen_connect(client_id, clean_session=True, keepalive=60, username=None, password=None, will_topic=None, will_qos=0, will_retain=False, will_payload=b"", proto_ver=4, connect_reserved=False, properties=b"", will_properties=b"", session_expiry=-1):
    ...
    if proto_ver == 5:
        if properties == b"":
            properties += testsupport.mqtt5_props.gen_uint16_prop(testsupport.mqtt5_props.PROP_RECEIVE_MAXIMUM, 20)

        if session_expiry != -1:
            properties += testsupport.mqtt5_props.gen_uint32_prop(testsupport.mqtt5_props.PROP_SESSION_EXPIRY_INTERVAL, session_expiry)

        properties = testsupport.mqtt5_props.prop_finalise(properties)
    ...
    if will_topic is not None:
        ...
        if proto_ver == 5:
            will_properties = testsupport.mqtt5_props.prop_finalise(will_properties)
    ...
    if proto_ver == 5:
        if property_helper == True:
            if properties is not None:
                properties = testsupport.mqtt5_props.gen_uint16_prop(testsupport.mqtt5_props.PROP_TOPIC_ALIAS_MAXIMUM, 10) \
                    + properties + testsupport.mqtt5_props.gen_uint16_prop(testsupport.mqtt5_props.PROP_RECEIVE_MAXIMUM, 20)
            else:
                properties = b""
        properties = testsupport.mqtt5_props.prop_finalise(properties)
