...
// publication on the NOAA website: See http://www.ngs.noaa.gov/PUBS_LIB/inverse.pdf
//
...
public class GeodeticCalculator
{
    ...
    /// <returns></returns>
    public GeoPoint2D CalculateEndingCoordinates(Ellipsoid ellipsoid, GeoPoint2D start, Angle startBearing, Distance distance, Angle endBearing) // out endBearing
    {
        ...
        double phi1 = start.getLatitude().inRadians();
        double alpha1 = startBearing.inRadians();
        double cosAlpha1 = Math.cos(alpha1);
        double sinAlpha1 = Math.sin(alpha1);
        double s = distance.inMetres();
        ...
        // build result
        Angle latitude = new Angle(phi2, AngleUnit.RADIANS);
        Angle longitude = new Angle(start.getLongitude().inRadians() + L, AngleUnit.RADIANS);

        endBearing = new Angle(alpha2, AngleUnit.RADIANS);
    ...
    /// <param name="distance">distance to travel (meters)</param>
    /// <returns></returns>
    public GeoPoint2D CalculateEndingCoordinates(Ellipsoid ellipsoid, GeoPoint2D start, Angle startBearing, Distance distance)
    {
        Angle endBearing = new Angle();

        return CalculateEndingCoordinates(ellipsoid, start, startBearing, distance, endBearing); // out endBearing
    }
    ...
    /// <returns></returns>
    public GeodeticCurve CalculateGeodeticCurve(Ellipsoid ellipsoid, GeoPoint2D start, GeoPoint2D end)
    {
        //
        ...
        double phi1 = start.getLatitude().inRadians();
        double lambda1 = start.getLongitude().inRadians();
        double phi2 = end.getLatitude().inRadians();
        double lambda2 = end.getLongitude().inRadians();
        ...
        if (!converged)
        {
            if (phi1 > phi2)
            {
                alpha1 = new Angle(180, AngleUnit.DEGREES);
                alpha2 = new Angle(0, AngleUnit.DEGREES);
            }
            else if (phi1 < phi2)
            {
                alpha1 = new Angle(0, AngleUnit.DEGREES);
                alpha2 = new Angle(180, AngleUnit.DEGREES);
            }
            else
            {
                alpha1 = new Angle();
        ...
        else
        {
            ...
            alpha1 = new Angle(radians, AngleUnit.RADIANS);

            // eq. 21
            radians = Math.atan2(cosU1 * Math.sin(lambda), (-sinU1cosU2 + cosU1sinU2 * Math.cos(lambda))) + Math.PI;
            if (radians < 0.0) radians += TwoPi;
            alpha2 = new Angle(radians, AngleUnit.RADIANS);
        }

        if (alpha1.inDegrees() >= 360.0) alpha1 = new Angle(alpha1.inDegrees() - 360.0, AngleUnit.DEGREES);
        if (alpha2.inDegrees() >= 360.0) alpha2 = new Angle(alpha2.inDegrees() - 360.0, AngleUnit.DEGREES);
    ...
    /// <returns></returns>
    public GeodeticMeasurement CalculateGeodeticMeasurement(Ellipsoid refEllipsoid, GeoPoint3D start, GeoPoint3D end)
    {
        // get the coordinates
        GeoPoint2D startCoords = start.getCoordinates();
        GeoPoint2D endCoords = end.getCoordinates();

        // calculate elevation differences
        double elev1 = start.getAltitude().inMetres();
        double elev2 = end.getAltitude().inMetres();
        double elev12 = (elev1 + elev2) / 2.0;

        // calculate latitude differences
        double phi1 = startCoords.getLatitude().inRadians();
        double phi2 = endCoords.getLatitude().inRadians();
